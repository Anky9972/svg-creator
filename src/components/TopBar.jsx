        ...
        {!authLoading && !user && (
          <>
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all bg-blue-600 hover:bg-blue-500 text-white active:scale-95 active:shadow-inner`}
            >
              Login to Save
            </button>
            ...
            {isLoginModalOpen && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                <div className={`p-6 w-80 rounded-2xl shadow-2xl border ${themeColors.sidebarSection} ${themeColors.border} relative`}>
                  <button 
                    onClick={() => setIsLoginModalOpen(false)} 
                    className={`absolute top-4 right-4 ${themeColors.textSecondary} hover:${themeColors.text}`}
                  >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                  
                  <h2 className={`text-xl font-bold mb-6 text-center ${themeColors.text}`}>
                    {isSignUp ? 'Create an Account' : 'Welcome Back'}
                  </h2>

                  <form onSubmit={(e) => { handleEmailAuth(e); setIsLoginModalOpen(false); }} className="mb-4">
                    <div className="space-y-4">
                      <div>
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email address" className={`w-full px-4 py-3 ${themeColors.inputBg} rounded-xl text-sm border ${themeColors.border} focus:border-blue-500 focus:outline-none transition-colors`} required />
                      </div>
                      <div>
                        <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" className={`w-full px-4 py-3 ${themeColors.inputBg} rounded-xl text-sm border ${themeColors.border} focus:border-blue-500 focus:outline-none transition-colors`} required />
                      </div>
                      {authError && <p className="text-red-400 text-xs text-center">{authError}</p>}
                      <button type="submit" className="w-full px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-bold transition-colors shadow-md shadow-blue-500/20 active:scale-95">
                        {isSignUp ? 'Sign Up' : 'Login'}
                      </button>
                    </div>
                    <div className="mt-4 text-center">
                      <button type="button" onClick={() => setIsSignUp(!isSignUp)} className={`text-sm ${themeColors.textSecondary} hover:text-blue-400 font-medium transition-colors`}>
                  ...
                  <div className="relative mb-6 mt-6">
                    ...
                    <div className="relative flex justify-center text-xs">
                      <span className={`px-4 ${themeColors.sidebarSection} ${themeColors.textSecondary} font-medium`}>Or continue with</span>
                    </div>
                  </div>
                  <button onClick={() => { loginWithGoogle(); setIsLoginModalOpen(false); }} className="w-full px-4 py-3 bg-white text-slate-800 hover:bg-slate-50 rounded-xl text-sm font-bold flex items-center justify-center gap-3 shadow-md border border-slate-200 active:scale-95 transition-all">
